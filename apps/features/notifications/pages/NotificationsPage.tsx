...
export function NotificationsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '440px 1fr', gap: 24 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#0C4A6E' }}>Target Audience *</label>
              <select
                value={audience}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#0C4A6E' }}>Notification Title *</label>
              <input
                type="text"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#0C4A6E' }}>Message Content *</label>
              <textarea
                placeholder="e.g. Order now from top rated pizzerias & get instant cashback in your wallet."
                value={body}
                onChange={(e) => setBody(e.target.value)}
                rows={4}
                style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid #BAE6FD', fontSize: 13, outline: 'none', resize: 'vertical', color: '#0C4A6E', backgroundColor: '#FFFFFF' }}
                required
              />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 12, fontWeight: 700, color: '#0C4A6E' }}>Action URL / Deep Link</label>
                <input
                  type="text"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 12, fontWeight: 700, color: '#0C4A6E' }}>Schedule Time</label>
                <select
                  value={scheduledTime}
          ...
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: 14, border: '1px solid #BAE6FD', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>
              Live User Notification Preview
            </h3>

            {/* Mobile Push Notification Mock Bubble */}
            <div style={{ backgroundColor: '#F0F9FF', borderRadius: 16, border: '1px solid #BAE6FD', padding: '16px 20px', boxShadow: '0 4px 12px rgba(2, 132, 199, 0.08)', maxWidth: 420 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 800, color: '#0284C7' }}>FOODIE MARKETPLACE</span>
                </div>
                <span style={{ fontSize: 11, color: '#0369A1' }}>now</span>
              </div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#0C4A6E' }}>
                {title || 'Your Push Notification Title'}
              </div>
              <div style={{ fontSize: 13, color: '#0369A1', marginTop: 4, lineHeight: 1.4 }}>
      ...
      {activeTab === 'CUSTOMER' && (
        <div style={{ backgroundColor: '#FFFFFF', borderRadius: 14, padding: 24, border: '1px solid #BAE6FD', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>Customer Push Broadcast Alerts</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
            {[
              ...
              <div key={idx} style={{ backgroundColor: '#F0F9FF', padding: 16, borderRadius: 10, border: '1px solid #BAE6FD' }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#0C4A6E' }}>{card.title}</div>
                <div style={{ fontSize: 12, color: '#0369A1', marginTop: 4, lineHeight: 1.4 }}>{card.desc}</div>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#0284C7', marginTop: 8 }}>{card.stats}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'RESTAURANT' && (
        <div style={{ backgroundColor: '#FFFFFF', borderRadius: 14, padding: 24, border: '1px solid #BAE6FD', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>Restaurant Partner Portal Alerts</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
            {[
      ...
      {activeTab === 'DELIVERY' && (
        <div style={{ backgroundColor: '#FFFFFF', borderRadius: 14, padding: 24, border: '1px solid #BAE6FD', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>Delivery Fleet Dispatch Alerts</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
            {[
      ...
      {activeTab === 'HISTORY' && (
        <div style={{ backgroundColor: '#FFFFFF', borderRadius: 14, border: '1px solid #BAE6FD', overflow: 'hidden' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #BAE6FD', backgroundColor: '#E0F2FE' }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>Notification Dispatch History Logs</h2>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
            ...
            <tbody>
              {history.map((row) => (
                <tr key={row.id} style={{ borderBottom: '1px solid #BAE6FD' }}>
                  <td style={{ padding: '16px 20px' }}>
                    <div style={{ fontWeight: 800, color: '#0C4A6E' }}>{row.title}</div>
                    <div style={{ fontSize: 12, color: '#0369A1', marginTop: 2 }}>{row.body}</div>
                  </td>
                  <td style={{ padding: '16px 20px' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, backgroundColor: '#E0F2FE', border: '1px solid #BAE6FD', color: '#0284C7', padding: '3px 8px', borderRadius: 4 }}>
                      {row.audience}
                    </span>
                  </td>
                  <td style={{ padding: '16px 20px', color: '#0369A1', fontSize: 12 }}>{row.sentTime}</td>
                  <td style={{ padding: '16px 20px', fontWeight: 700, color: '#0C4A6E' }}>{row.recipientsCount.toLocaleString()}</td>
                  <td style={{ padding: '16px 20px', color: '#0C4A6E', fontWeight: 700 }}>{row.deliveryRate}</td>
                  <td style={{ padding: '16px 20px', color: '#0C4A6E', fontWeight: 700 }}>{row.openRate}</td>
                  <td style={{ padding: '16px 20px' }}>
                    <span style={{ backgroundColor: '#0284C7', color: '#FFFFFF', fontSize: 11, fontWeight: 800, padding: '4px 10px', borderRadius: 20 }}>
                      {row.status}
                    </span>
                  </td>
                </tr>
