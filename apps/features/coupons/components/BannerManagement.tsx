...
export function BannerManagement() {
    ...
    const startEdit = (banner: PromotionBanner) => {
        ...
        window.scrollTo({ top: 0, behavior: 'smooth' }); // form will now be at the bottom, so it might need different scroll behavior, but top works initially or bottom.
    };
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 32 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {isLoading ? (
                    ...
                    banners.map((banner: PromotionBanner, index: number) => {
                        const bgColor = VIBRANT_COLORS[index % VIBRANT_COLORS.length];
                        const activeEmojis = EMOJIS[index % EMOJIS.length];
                        return (
                            <div key={banner.id} style={{ display: 'flex', flexDirection: 'column', gap: 12, backgroundColor: '#FFFFFF', padding: 16, borderRadius: 12, border: '1px solid #E4E4E7' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <h4 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Banner #{index + 1}</h4>
                                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                        <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 12, backgroundColor: banner.status === 'ACTIVE' ? '#D1FAE5' : '#FEF2F2', color: banner.status === 'ACTIVE' ? '#065F46' : '#991B1B', fontWeight: 700 }}>
                                            {banner.status}
                                        </span>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ position: 'absolute', top: 20, left: 200, fontSize: 30, opacity: 0.8 }}>{activeEmojis[2]}</div>
                                    <div style={{
                                        flex: 1,
                                        padding: 20,
                                        display: 'flex',
                                        ...
                                        <div style={{
                                            ...
                                            padding: '8px 16px',
                                            borderRadius: 20,
                                            marginTop: banner.subtitle ? 0 : 16,
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, backgroundColor: '#FAFAFA', padding: 24, borderRadius: 12, border: '1px solid #E4E4E7' }}>
                ...
                <div style={{
                    ...
                    borderRadius: 20,
                    overflow: 'hidden',
                    backgroundColor: VIBRANT_COLORS[0],
                    position: 'relative',
                    boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
                    display: 'flex',
                    marginTop: 16
                }}>
                    <div style={{ position: 'absolute', top: -10, right: 10, fontSize: 45, opacity: 0.8 }}>{EMOJIS[0][0]}</div>
                    <div style={{ position: 'absolute', bottom: 10, right: 60, fontSize: 35, opacity: 0.8 }}>{EMOJIS[0][1]}</div>
                    <div style={{ position: 'absolute', top: 20, left: 200, fontSize: 30, opacity: 0.8 }}>{EMOJIS[0][2]}</div>
                    <div style={{
                        flex: 1,
                        padding: 20,
                        display: 'flex',
