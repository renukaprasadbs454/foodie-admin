    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
            ...
            (st) => (
              <button
                ...
                style={{
                  ...
                  boxShadow: statusFilter === st ? '0 2px 6px rgba(2, 132, 199, 0.2)' : 'none',
                }}
              >
                {st}
              </button>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
          ...
          <tbody>
            {filteredOrders.map((o) => (
              <tr key={o.id} style={{ borderBottom: '1px solid #E0F2FE' }}>
                <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0369A1' }}>{o.orderNumber}</td>
                <td style={{ padding: '14px 16px' }}>
                  <div style={{ fontWeight: 700, color: '#0369A1' }}>{o.customerName}</div>
                  <div style={{ fontSize: 11, color: '#0284C7' }}>{o.customerPhone}</div>
                </td>
                ...
                <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0369A1' }}>₹{o.totalAmount.toFixed(2)}</td>
                <td style={{ padding: '14px 16px' }}>
                  <span
                    style={{
                      padding: '4px 10px',
                ...
                <td style={{ padding: '14px 16px', fontSize: 12, color: '#0284C7' }}>
                  Picker: <strong style={{ color: '#0369A1' }}>{o.assignedPicker || 'Unassigned'}</strong>
                  <br />
                  Packer: <strong style={{ color: '#0369A1' }}>{o.assignedPacker || 'Unassigned'}</strong>
                </td>
                <td style={{ padding: '14px 16px' }}>
      ...
      {selectedOrder && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 24, maxWidth: 600, width: '100%', border: '1px solid #BAE6FD' }}>
            <h2 style={{ fontSize: 18, fontWeight: 900, color: '#0369A1', margin: '0 0 16px' }}>
              Order Details — {selectedOrder.orderNumber}
            </h2>

            <div style={{ fontSize: 13, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16, color: '#0369A1' }}>
            ...
            <h3 style={{ fontSize: 14, fontWeight: 800, color: '#0369A1', marginBottom: 8 }}>Order Items</h3>
            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', border: '1px solid #BAE6FD', borderRadius: 8 }}>
              {selectedOrder.items.map((i) => (
                <li key={i.id} style={{ padding: '10px 14px', borderBottom: '1px solid #E0F2FE', display: 'flex', justifyContent: 'space-between', fontSize: 12, backgroundColor: '#F0F9FF' }}>
                  <div>
                    <strong style={{ color: '#0369A1' }}>{i.productName}</strong> ({i.sku})
                    <div style={{ fontSize: 11, color: '#0284C7' }}> Location: {i.shelfLocation}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#0369A1', fontWeight: 700 }}>Qty: {i.quantityRequested} | Picked: {i.quantityPicked}</div>
                    <span style={{ fontSize: 10, fontWeight: 800, color: '#0369A1', backgroundColor: '#E0F2FE', padding: '2px 6px', borderRadius: 4, border: '1px solid #BAE6FD' }}>{i.status}</span>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #0284C7 0%, #0369A1 100%)', color: '#FFFFFF', border: 'none', padding: '8px 16px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
              >
                Close Window
              </button>
            </div>
