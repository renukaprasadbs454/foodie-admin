    ...
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, border: '1px solid #BAE6FD', boxShadow: '0 1px 3px rgba(2, 132, 199, 0.05)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
          ...
          <tbody>
            {staffList.map((s) => (
              <tr key={s.id} style={{ borderBottom: '1px solid #F0F9FF' }}>
                <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0284C7' }}>{s.name}</td>
                <td style={{ padding: '14px 16px' }}>
                  <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 800, backgroundColor: '#E0F2FE', color: '#0284C7' }}>
                    {s.role}
                  </span>
                </td>
                <td style={{ padding: '14px 16px', color: '#0C4A6E' }}>
                  {s.phone}
                  <div style={{ fontSize: 11, color: '#0369A1' }}>{s.email}</div>
                </td>
                <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0C4A6E' }}>{s.activeTasksCount} Active Orders</td>
                <td style={{ padding: '14px 16px' }}>
