...
export function DarkstoreDashboardPage() {
  ...
  const kpiCards = [
    { title: 'Total Orders', value: metrics.totalOrders, color: '#0284C7', icon: '', bg: '#E0F2FE' },
    { title: 'New Orders', value: metrics.newOrders, color: '#D97706', icon: '', bg: '#FEF3C7' },
    { title: 'Picking in Progress', value: metrics.ordersBeingPicked, color: '#0284C7', icon: '', bg: '#E0F2FE' },
    { title: 'Ready for Dispatch', value: metrics.ordersReadyForDispatch, color: '#0369A1', icon: '', bg: '#F0F9FF' },
    { title: 'Completed Orders', value: metrics.completedOrders, color: '#0284C7', icon: '', bg: '#E0F2FE' },
    { title: 'Cancelled Orders', value: metrics.cancelledOrders, color: '#DC2626', icon: '', bg: '#FEE2E2' },
    { title: 'Low Stock SKU', value: metrics.lowStockProducts, color: '#EA580C', icon: '', bg: '#FFEDD5' },
    { title: 'Out of Stock SKU', value: metrics.outOfStockProducts, color: '#DC2626', icon: '', bg: '#FEE2E2' },
    { title: 'Total Catalog Products', value: metrics.totalProducts, color: '#0C4A6E', icon: '', bg: '#F0F9FF' },
    { title: "Today's Revenue", value: `₹${metrics.todaysRevenue.toFixed(2)}`, color: '#0284C7', icon: '', bg: '#E0F2FE' },
    { title: 'Average Order Value', value: `₹${metrics.averageOrderValue.toFixed(2)}`, color: '#0369A1', icon: '', bg: '#F0F9FF' },
    { title: 'Pending Actions', value: metrics.pendingActionsCount, color: '#D97706', icon: '', bg: '#FEF3C7' },
  ];
    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {kpiCards.map((card, idx) => (
          ...
          >
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#0369A1', textTransform: 'uppercase' }}>{card.title}</div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, border: '1px solid #BAE6FD', boxShadow: '0 1px 3px rgba(2, 132, 199, 0.05)', padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: '#0C4A6E', margin: 0 }}>
             Active Quick-Commerce Order Queue
          </h2>
          <Link href="/darkstore-admin/orders" style={{ fontSize: 13, fontWeight: 700, color: '#0284C7', textDecoration: 'none' }}>
            View All Orders →
          </Link>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
            ...
            <tbody>
              {activeOrders.map((order) => (
                <tr key={order.id} style={{ borderBottom: '1px solid #F0F9FF' }}>
                  <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0284C7' }}>{order.orderNumber}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ fontWeight: 700, color: '#0C4A6E' }}>{order.customerName}</div>
                    <div style={{ fontSize: 11, color: '#0369A1' }}>{order.customerPhone}</div>
                  </td>
                  <td style={{ padding: '14px 16px', color: '#64748B', maxWidth: 220 }}>{order.deliveryAddress}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <span
                      style={{
                        padding: '4px 8px',
                        ...
                        color: order.priority === 'HIGH' ? '#DC2626' : '#0284C7',
                      }}
                    >
                      {order.priority}
                    </span>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <span
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 800,
                        backgroundColor: order.status === 'NEW' ? '#FEF3C7' : '#E0F2FE',
                        color: order.status === 'NEW' ? '#B45309' : '#0284C7',
                      }}
                    >
                      {order.status}
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px', color: '#0C4A6E' }}>{order.assignedPicker || 'Unassigned'}</td>
                  <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0284C7' }}>₹{order.totalAmount.toFixed(2)}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <Link
