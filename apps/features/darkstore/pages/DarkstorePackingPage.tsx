    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#0369A1', margin: '4px 0 0' }}>
          Active Packing Order: <strong style={{ color: '#0284C7' }}>{orderNumber}</strong> | Assigned Packer: <strong style={{ color: '#0C4A6E' }}>{assignedPacker}</strong>
        </p>
      </div>

      <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 24, border: '1px solid #BAE6FD', boxShadow: '0 1px 3px rgba(2, 132, 199, 0.05)', marginBottom: 24 }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 24px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((it, idx) => (
            <li key={idx} style={{ padding: 12, backgroundColor: '#F0F9FF', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, border: '1px solid #BAE6FD' }}>
              <div>
                <span style={{ fontWeight: 800, color: '#0284C7' }}> {it.name}</span>
                <span style={{ fontSize: 11, color: '#0369A1', marginLeft: 8 }}>Qty: {it.qty}</span>
              </div>
              <span style={{ fontSize: 11, fontWeight: 800, color: '#0284C7', backgroundColor: '#E0F2FE', padding: '2px 8px', borderRadius: 6, border: '1px solid #BAE6FD' }}>
                VERIFIED
              </span>
            </li>
          ))}
        ...
        {!isPacked ? (
          ...
          <div style={{ backgroundColor: '#E0F2FE', color: '#0284C7', padding: 16, borderRadius: 8, textAlign: 'center', fontWeight: 800, border: '1px solid #BAE6FD' }}>
