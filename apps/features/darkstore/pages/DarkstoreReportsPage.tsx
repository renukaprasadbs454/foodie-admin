  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button
            type="button"
            onClick={exportCSV}
            style={{ backgroundColor: '#0369A1', color: '#FFFFFF', padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer' }}
          >
            Export CSV 
          </button>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        <div style={{ backgroundColor: '#FFFFFF', padding: 20, borderRadius: 12, border: '1px solid #BAE6FD' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#0284C7' }}>AVG PICK TIME / ORDER</div>
          <div style={{ fontSize: 26, fontWeight: 900, color: '#0369A1', marginTop: 4 }}>3.2 mins</div>
          <div style={{ fontSize: 11, color: '#15803D', marginTop: 4 }}> Sub 5-min target met</div>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', padding: 20, borderRadius: 12, border: '1px solid #BAE6FD' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#0284C7' }}>AVG PACK TIME / ORDER</div>
          <div style={{ fontSize: 26, fontWeight: 900, color: '#0369A1', marginTop: 4 }}>1.8 mins</div>
          <div style={{ fontSize: 11, color: '#15803D', marginTop: 4 }}> Sub 2-min target met</div>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', padding: 20, borderRadius: 12, border: '1px solid #BAE6FD' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#0284C7' }}>DISPATCH PREP SLA</div>
          <div style={{ fontSize: 26, fontWeight: 900, color: '#0369A1', marginTop: 4 }}>98.4%</div>
          <div style={{ fontSize: 11, color: '#15803D', marginTop: 4 }}> 10-min store exit SLA</div>
        </div>
      </div>

      <div style={{ backgroundColor: '#FFFFFF', borderRadius: 12, padding: 20, border: '1px solid #BAE6FD' }}>
        <h2 style={{ fontSize: 16, fontWeight: 800, color: '#0369A1', margin: '0 0 16px' }}>
          Top Selling Darkstore Products Today
        </h2>
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <li style={{ padding: 12, backgroundColor: '#F0F9FF', borderRadius: 8, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            <div><strong>1. Amul Taaza Toned Fresh Milk 500ml</strong> (SKU: MILK-AMUL-500ML)</div>
            <div style={{ fontWeight: 800, color: '#0369A1' }}>42 units sold</div>
          </li>
          <li style={{ padding: 12, backgroundColor: '#F0F9FF', borderRadius: 8, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            <div><strong>2. Coca-Cola Zero Sugar Can 330ml</strong> (SKU: COKE-ZERO-330ML)</div>
            <div style={{ fontWeight: 800, color: '#0369A1' }}>28 units sold</div>
          </li>
          <li style={{ padding: 12, backgroundColor: '#F0F9FF', borderRadius: 8, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            <div><strong>3. Britannia Brown Bread Whole Wheat 400g</strong> (SKU: BREAD-BRIT-400G)</div>
            <div style={{ fontWeight: 800, color: '#0369A1' }}>18 units sold</div>
          </li>
        </ul>
      </div>
    </div>
