...
export function GlobalSearchModal({ isOpen, onClose }: GlobalSearchModalProps) {
  ...
  const results: SearchResultItem[] = React.useMemo(() => {
    ...
    if (globalData?.restaurants) {
      globalData.restaurants.forEach((r) => {
        res.push({
          ...
          category: 'Restaurant',
          title: r.name,
          subtitle: `${r.cuisineType ?? 'Restaurant'} • Rating ${r.rating ?? '4.5'} • ${r.isAvailable !== false ? 'Active' : 'Inactive'
            }`,
    ...
    if (globalData?.foodItems) {
      globalData.foodItems.forEach((f) => {
        res.push({
          ...
          category: 'Food Item',
          title: f.name,
          subtitle: `${f.categoryName ?? 'Menu Item'} • ₹${f.basePrice?.toFixed(2) ?? '0.00'}${f.isVeg ? ' • Veg' : ''
            }`,
          url: `/restaurants`,
          icon: '',
        });
    ...
    if (debouncedQuery) {
      ...
      MOCK_ENTITIES.forEach((item) => {
        if (
          ...
          if (!res.some((r) => r.title.toLowerCase() === item.title.toLowerCase())) {
            res.push(item);
          }
        }
      });
    ...
    return res;
  }, [globalData, orderData, debouncedQuery]);
    ...
    >
      <div
        style={{
          ...
          borderRadius: 16,
          boxShadow: '0 20px 25px -5px rgba(2, 132, 199, 0.2), 0 10px 10px -5px rgba(2, 132, 199, 0.08)',
      ...
      >
        ...
        {/* Results Body */}
        <div style={{ maxHeight: '380px', overflowY: 'auto', padding: '12px 8px', backgroundColor: '#FFFFFF' }}>
          {query.trim() === '' ? (
            <div style={{ padding: '36px 16px', textAlign: 'center', color: '#0284C7', fontSize: 13, lineHeight: 1.6 }}>
          ...
          ) : isSearchingGlobal || isFetchingOrder ? (
            <div style={{ padding: '36px 16px', textAlign: 'center', color: '#0369A1', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
              <span className="pulse-live" style={{ width: 12, height: 12, borderRadius: '50%', backgroundColor: '#0284C7' }} />
              Fetching backend search results...
            </div>
          ) : isGlobalError ? (
            <div style={{ padding: '24px 16px', textAlign: 'center', color: '#0369A1', fontSize: 13, backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: 8, margin: '8px' }}>
              Failed to fetch search results from server. Please check your backend connection.
            </div>
          ) : results.length === 0 ? (
            <div style={{ padding: '32px 16px', textAlign: 'center', color: '#0284C7', fontSize: 14 }}>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: '#0369A1' }}>{item.title}</span>
                      <span
                        style={{
                          fontSize: 10,
                          fontWeight: 700,
                    ...
                    <div style={{ fontSize: 12, color: '#0284C7', marginTop: 2 }}>{item.subtitle}</div>
                  </div>
        ...
        {/* Footer shortcuts */}
        <div style={{ padding: '10px 20px', backgroundColor: '#F0F9FF', borderTop: '1px solid #BAE6FD', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: '#0284C7' }}>
